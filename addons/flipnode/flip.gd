...
static func is_node_editable(node: Node) -> bool:
...
func create_polygon_circle(center: Vector2, diameter: float = 32.0, vertex_count := 32) -> PackedVector2Array:
	var vertices := []
...
static func catmull_rom(points: PackedVector2Array, max_length := 10.0) -> PackedVector2Array:
	if points.size() < 4:
	...
	for i in range(points.size() - 3):
		...
		var num_segments := minf(ceilf(p1.distance_to(p2) / max_length), 8)
...
static func catmull_rom_closed(points: PackedVector2Array, max_length := 10.0) -> PackedVector2Array:
	if points.size() < 4:
		return PackedVector2Array(points)
...
static func merge_by_distance(points: PackedVector2Array, min_distance := 2.0) -> PackedVector2Array:
...
static func warp_ease(t: float, mode: int) -> float:
	match mode:
		WARP_EASE_SMOOTH:
			return ease(t, -1.5)
		WARP_EASE_SHARP:
